 ...
 *
 * On Windows, path.relative() returns backslash-separated paths, but our
 * exclusion regex patterns use forward slashes only. Normalizing here
 * ensures matching works on all platforms.
 *
 * @param {string} p — path string (may contain backslashes on Windows)
 * @returns {string}
 */
 ...
 *
 * Normalizes backslashes to forward slashes before matching, so this works
 * correctly on Windows where path.relative() returns backslash paths.
 *
 * @param {string} relativePath — path relative to BASE_DIR (any separator)
