...
const DONE_VIA_EDIT_NOTICE =
  "NOTE: backlog_task_edit(status=\"Done\") was automatically upgraded " +
  "to backlog_task_complete. The task has been moved to the completed folder.";
 ...
 *  2. Auto-chains `task_edit` with status "Done" → sets status then
 *     calls `task_complete` to move the task to completed/
 *  3. Forwards everything else to the upstream MCP client
 *
...
function createToolHandler(client) {
  return async function handleTool(toolName, args) {
    ...
    if (isDoneViaEdit(toolName, args)) {
      // Let the status update go through first (upstream task_complete
      // requires the task to already be in "Done" status).
      await client.callTool({ name: "task_edit", arguments: args });
      ...
      // Prepend notice so the agent learns the correct tool for next time
      const upstream = completeResult?.content?.[0]?.text ?? "";
      return {
        content: [{
          type: "text",
          text: DONE_VIA_EDIT_NOTICE + "\n\n" + upstream,
        }],
      };
    }
