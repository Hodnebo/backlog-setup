...
function isDoneViaEdit(toolName, args) {
  if (toolName !== "backlog_task_edit") return false;
...
function rewriteToolList(listResult) {
  ...
  const tools = listResult.tools.map((tool) => {
    ...
    if (tool.name === "backlog_task_edit") {
    ...
    if (tool.name === "backlog_task_complete") {
      const patched = structuredClone(tool);
 ...
 *  2. Blocks `backlog_task_edit` with status "Done" → returns error
